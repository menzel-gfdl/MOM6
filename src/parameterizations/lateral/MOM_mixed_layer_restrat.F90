...
subroutine mixedlayer_restrat_register_restarts(HI, param_file, C, CS, restart_CS)
  ! Arguments
  ...
  if (CS%MLE_MLD_decay_time>0. .or. CS%MLE_MLD_decay_time2>0.) then
    ...
    vd = var_desc("MLD_MLE_filtered","m","Time-filtered MLD for use in MLE", &
                  hor_grid='h', z_grid='1')
    call register_restart_field(CS%MLD_filtered, vd, G, .false., restart_CS)
  endif
  if (CS%MLE_MLD_decay_time2>0.) then
    ...
    vd = var_desc("MLD_MLE_filtered_slow","m","c Slower time-filtered MLD for use in MLE", &
                  hor_grid='h', z_grid='1')
    call register_restart_field(CS%MLD_filtered_slow, vd, G, .false., restart_CS)
  endif
