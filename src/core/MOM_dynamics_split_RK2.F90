...
subroutine register_restarts_dyn_split_RK2(HI, GV, G, param_file, CS, restart_CS, uh, vh)
  ...
  call register_restart_field(CS%eta, vd%name, .false., G, restart_CS)

  vd = var_desc("u2","m s-1","Auxiliary Zonal velocity",'u','L')
  call register_restart_field(CS%u_av, vd%name, .false., G, restart_CS)

  vd = var_desc("v2","m s-1","Auxiliary Meridional velocity",'v','L')
  call register_restart_field(CS%v_av, vd%name, .false., G, restart_CS)

  vd = var_desc("h2",thickness_units,"Auxiliary Layer Thickness",'h','L')
  call register_restart_field(CS%h_av, vd%name, .false., G, restart_CS)
  ...
  call register_restart_field(CS%diffv, vd%name, .false., G, restart_CS)
