...
subroutine call_tracer_register(HI, GV, param_file, G, CS, tr_Reg, restart_CS)
...
!  for some reason.  This then overrides the run-time selection from above.
  if (CS%use_USER_tracer_example) CS%use_USER_tracer_example = &
    USER_register_tracer_example(HI, GV, param_file, G, CS%USER_tracer_example_CSp, &
                                 tr_Reg, restart_CS)
  if (CS%use_DOME_tracer) CS%use_DOME_tracer = &
    register_DOME_tracer(HI, GV, param_file, G, CS%DOME_tracer_CSp, &
                         tr_Reg, restart_CS)
  if (CS%use_ISOMIP_tracer) CS%use_ISOMIP_tracer = &
    register_ISOMIP_tracer(HI, GV, param_file, G, CS%ISOMIP_tracer_CSp, &
                           tr_Reg, restart_CS)
  if (CS%use_ideal_age) CS%use_ideal_age = &
    register_ideal_age_tracer(HI, GV, param_file, G, CS%ideal_age_tracer_CSp, &
                              tr_Reg, restart_CS)
  if (CS%use_regional_dyes) CS%use_regional_dyes = &
    register_dye_tracer(HI, GV, param_file, G, CS%dye_tracer_CSp, &
                        tr_Reg, restart_CS)
  if (CS%use_oil) CS%use_oil = &
    register_oil_tracer(HI, GV, param_file, G, CS%oil_tracer_CSp, &
                        tr_Reg, restart_CS)
  if (CS%use_advection_test_tracer) CS%use_advection_test_tracer = &
    register_advection_test_tracer(HI, GV, param_file, G, CS%advection_test_tracer_CSp, &
                                   tr_Reg, restart_CS)
  if (CS%use_OCMIP2_CFC) CS%use_OCMIP2_CFC = &
    register_OCMIP2_CFC(HI, GV, param_file, G, CS%OCMIP2_CFC_CSp, &
                        tr_Reg, restart_CS)
#ifdef _USE_GENERIC_TRACER
  if (CS%use_MOM_generic_tracer) CS%use_MOM_generic_tracer = &
    register_MOM_generic_tracer(HI, GV, param_file, G, CS%MOM_generic_tracer_CSp, &
                                tr_Reg, restart_CS)
#endif
  if (CS%use_pseudo_salt_tracer) CS%use_pseudo_salt_tracer = &
    register_pseudo_salt_tracer(HI, GV, param_file, G, CS%pseudo_salt_tracer_CSp, &
                                tr_Reg, restart_CS)
  if (CS%use_boundary_impulse_tracer) CS%use_boundary_impulse_tracer = &
    register_boundary_impulse_tracer(HI, GV, param_file, G, CS%boundary_impulse_tracer_CSp, &
                                     tr_Reg, restart_CS)
  if (CS%use_dyed_obc_tracer) CS%use_dyed_obc_tracer = &
    register_dyed_obc_tracer(HI, GV, param_file, G, CS%dyed_obc_tracer_CSp, &
                             tr_Reg, restart_CS)
